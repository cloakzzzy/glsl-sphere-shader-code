...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 rayOrigin = vec3(0.f, 2.0f, 0.f);
    
    float radius [] = float[](1.f,1.f,1.f);
    vec3 spherepos [] = vec3[](vec3(3.f,0.f,5.f), vec3(0.0f ,0.0f ,2.0f),vec3( 0.5,0.5,5.f));
    vec3 spherecol []= vec3[](vec3(0.f,0.f,1.f), vec3(1.f,0.f,0.f), vec3(0.f,1.f,0.f));
     //                           blue                   red                green
    float t [] = float[](0.f,0.f,0.f);
    ...
    //if there are no intersections make it white ( if all values in t array are the same
    ...
    if (eq == true)
    {
        ...
        fragColor = vec4(vec3(backcol), 0.f);
        return;
    }
